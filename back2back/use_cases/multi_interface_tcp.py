...
For this use-case the parent ByteBlower Port is configurable for each client.
This script serves as a comparisson to the multi_client TCP example.
...
configuration = {
    ...
    'server_address':
    'byteblower-tutorial-3100.lab.byteblower.excentis.com',
...
class Example:
    def __init__(self, **kwargs):
        ...
        for a_client in kwargs['http_clients']:
            ...
            self.http_client_configs.append({
                ...
                'byteblower_port': a_client['client_bb_port']
            })

        self.server = None
        self.server_bb_port = None
        self.client_bb_ports = [] 
    ...
    def run(self):
        ...
        byteblower_instance.PortsStartAll()
        ...
        while time_elapsed <= (extra_time + max_duration):
            ...
            # - Error
            finished_states = [
                HTTPRequestStatus.Error, HTTPRequestStatus.Finished
            ]
    ...
    def process_http_client(cls, http_client):
        ...
        return [
            tx_bytes, rx_bytes,
            avg_throughput.bitrate(), min_congestion, max_congestion,

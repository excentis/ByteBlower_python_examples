...
"""
 ...
 The main goal of the script is teaching how the API works. We will print out
 the results to screen. For a more involved demo we suggest to look into the
 demo folder (toplevel)

 Polling the Network Info is one of the two ways to get the Wi-Fi statistics.
 The Wi-Fi monitor is the other one. They have following differences:

 * The polling method returns the Wi-Fi statistics cached in the  MeetingPoint.
   These values are updated every several heartbeats. It can take up to a
   minute for a change to propagate.

 * With the Polling method, the Wi-Fi statistics can be retrieved without
   locking the Wireless Endpoint. The WEP remains available for anyone to use.

 * The polling methods return immediately. There are no blocking calls.
   In addition all error handling is delegated to the MeetingPoint. This makes
   the polling approach much easier to use.

 * During a testrun the Wireless Endpoint isn't heartbeating.
   There are thus no updates of the results at this time. You can still
   retrieve the old values though.

 We recommend using the polling method when indicative Wi-Fi statistics are
 sufficient (e.g. displaying the value to a user), when there's only interest
 in changes over several minutes (e.g. monitoring).
...
configuration = {
    ...
    # *must* be registered to the meetingpoint configured by meetingpoint_address.
    ...
    # The script will pick another interface when this one isn't found.
